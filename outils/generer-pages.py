...
def titre_bien(b):
    ...
    quartier = (b.get("quartier") or "").strip()
    if quartier and quartier.lower() != b["commune"].lower():
        lieu = f'{quartier}, {b["commune"]}'
    else:
        lieu = b["commune"]
    if b["region"].lower() != b["commune"].lower():
        lieu += f' ({b["region"]})'
...
def description_bien(b):
    ...
    bout = [f'{b["type"]} {action} à {b["commune"]} ({b["region"]})',
            surface(b), prix]
    if b.get("chambres"):
...
def donnees_structurees(b, photos, url):
    ...
    if b.get("quartier"):
        d["address"]["streetAddress"] = b["quartier"]
...
def page_bien(b, photos):
    nom = f"{slug(b['type'])}-{'a-vendre' if b['operation'] == 'Vente' else 'a-louer'}-{slug(b['commune'])}-{slug(b['ref'])}.html"
    url = f"{SITE}/bien/{nom}"
    titre = titre_bien(b)
    ...
    wa = f"https://wa.me/{TEL.lstrip('+')}?text=" + urllib.parse.quote(
        f"Bonjour, je suis intéressé(e) par le bien {b['ref']} — {b['type']} à {b['commune']}.")
...
<main>
  <nav class="fil" aria-label="Fil d'Ariane">
    <a href="../{ACCUEIL}">Tous les biens</a> › {esc(b["type"])} › {esc(b["commune"])}
  </nav>

  <h1>{esc(titre)}</h1>
  <p class="lieu">{esc((b.get("quartier") + ", ") if b.get("quartier") else "")}{esc(b["commune"])}, région de {esc(b["region"])}</p>
  <p class="prix">{prix}</p>
  ...
  <section class="contact">
    ...
    <p>Référence {esc(b["ref"])} — visites sur rendez-vous à {esc(b["commune"])}. Nous répondons rapidement.</p>
    <div class="actions">
  ...
  <a class="retour" href="../{ACCUEIL}">← Voir tous nos biens à Dakar et Thiès</a>
</main>
...
def main():
    ...
    urls = []
    for b in biens:
        nom, html = page_bien(b, par_bien.get(b["id"], []))
